...
class Menu extends FlappyGame {
    constructor() {
        super();
        this.bird = new Bird({bloodable: false});
        this.name = 'menu';
    }
...
class PvpGame extends FlappyGame {
    constructor({ playerID, gameRecord } = {}) {
        super();
        this.name = 'pvp game';
        this.level = 5;
        this.bloodBar = new BloodBar(100);
        this.queue_obstacles = gameRecord.queue_obstacles;
        this.bird = new Bird({ bloodBar: this.bloodBar });
        this.opponent_bird = new Bird({ x: 200, y: gameRecord.input[0].birdY, birdName: playerID, bloodable:false });
        ...
        this.onDestroy = function() {
            ...
            $('#menu_button').fadeIn(() => {
                ...
                game = new Menu();
                Object.assign(game.bird, { y: Height });
            });
        }
        ...
        let start_index = 1;
        for (var i in this.queue_obstacles) {
        ...
        // console.log(this.queue_obstacles)

        this.queue_obstacles.push(new FrameWait({ amount: 120 }));
        ...
        for (var i = start_index; i <= start_index + 100; i++) {
            config.index = i;
            ...
            if (Math.round(random(1, 2)) == 1) {
                ...
                this.queue_obstacles.push(new FrameWait({ amount: 60 }));
    ...
    lose() {
        ...
        this.recordGame.input.push({ name: 'bird.lose', frame: this.frameCount, birdY: this.bird.y });
    }

    keyPressed() {
        this.bird.fly();
        this.recordGame.input.push({ name: 'bird.fly', frame: this.frameCount, birdY: this.bird.y });
    }
...
class ClassicGame extends FlappyGame {
    constructor({} = {}) {
        ...
        // this.recordGame.queue_obstacles = [...this.queue_obstacles];
        this.recordGame.queue_obstacles = JSON.parse(JSON.stringify(this.queue_obstacles));
        // console.log(this.recordGame.queue_obstacles)
        // debugger;
    }
